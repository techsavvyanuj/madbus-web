...
export default function AuthModal({ open, onClose, bookingFor }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (data.Result === "true") {
        // Backend bug safeguard: Hostinger API sometimes returns true but fails to insert the user (returning UserLogin: null)
        if (mode === 'signup' && data.UserLogin === null) {
          setErrorMsg('Backend database error: Unable to save account on the server. Please check the Hostinger server database schema.')
          return
        }

        setDone(true)
        // Store user info if needed (only on login, or if signup returns the user obj)
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 animate-fade-in">
        <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
          ...
          <h2 className="font-display font-bold text-xl text-slate-800 mb-2">
            {bookingFor ? `Booking confirmed!` : mode === 'login' ? 'Welcome back!' : 'Account Created Successfully!'}
          </h2>
          <p className="text-slate-500 text-sm leading-relaxed">
            {bookingFor
              ? `Your ticket for ${bookingFor} has been reserved. Check your email for confirmation.`
              : mode === 'login' 
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="relative px-6 pt-6 pb-8 overflow-hidden shrink-0" style={{ minHeight: '140px' }}>
          ...
          <div
            ...
            style={{ backgroundImage: "url('/2489.jpg')" }}
          />
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/70 text-sm mt-1">
              {mode === 'login' ? 'Login to access your tickets & offers' : 'Join 5 million+ happy travellers'}
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          {mode === 'login' && (
            <div className="text-right -mt-2">
              <button className="text-xs text-brand-500 font-semibold hover:underline">Forgot password?</button>
            </div>
          )}
          ...
          >
            {loading ? (
              ...
              mode === 'login' ? 'Login to MadBus' : 'Create Account'
            )}
          </button>
